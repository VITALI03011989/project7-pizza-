...
import { TSortingType } from "../Pages/Home";

type SortProps={
  value:any;
  onChangeSort:(i:TSortingType)=>void
}

const Sort:React.FC<SortProps> = ({value,onChangeSort}) => {
    const [open,setOpen]=React.useState(false)
    const list:TSortingType[] = [
      {name:'популярности(возр)', sort:'rating'},
      {name:'популярности(убыв)', sort:'-rating'},
      {name:'цене(возр)', sort:'price'},
      {name:'цене(убыв)', sort:'-price'}, 
      {name:'aлфавит(возр)', sort:'title'},
      {name:'aлфавит(убыв)', sort:'-title'},
    ]
   

    const onClickListItem = (i:TSortingType) => {
        onChangeSort(i)
        setOpen(false)
    }
